...
for i = 1 : node_num
    ...
    for j = 1 : diff
        [single_t_source, single_c_source] = node_source_consume(eval(['s', num2str(i + j)]));
        [single_t_time, single_c_time] = node_time_consume(eval(['s', num2str(i + j)]));
        s_exist_time(i + j) = s_source(i + j) / single_c_source * single_c_time;
    end
